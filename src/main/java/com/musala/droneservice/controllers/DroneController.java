...
public class DroneController {
    ...
    @GetMapping("/ds/get-loaded-medications/{drone_id}")
    ...
    * */
    ...
    public ApiResponse<Object> getDrones() {
        return new ApiResponse<>("Drone loaded successfully", HttpStatus.OK, droneService.getDrones());
    }

    //Fetches all the deliveries of a drone that are still pending.
    @GetMapping("/ds/get-deliveries/{drone_id}")
    ...
    public ApiResponse<Object> updateDrones(@PathVariable("drone_id") Long droneId, @RequestBody Drone drone) {
        ...
        if (drone.getState() != null) {
            ...
            if (fetchedDrone.getBatteryCapacity() < 25 && drone.getState() == DroneState.LOADING) {
                return new ApiResponse<>("Drone battery is below 25%, Please charge", HttpStatus.OK, fetchedDrone);
            }
            fetchedDrone.setState(drone.getState());
        }
        //Ensure the battery is not 0 before setting.
        ...
        return new ApiResponse<>("Drone data updated", HttpStatus.OK, fetchedDrone);
    }
    ...
    public ApiResponse<Object> deleteDrones(@PathVariable("drone_id") Long droneId) {
        //Ensure drone exist before deleting
    ...
    //load a specific drone and
    @PostMapping("/ds/load-drone/{drone_id}/{medication_id}")
    ...
    //Change drone state
    @PutMapping("/ds/change-drone-status/{drone_id}/{state}")
    public ApiResponse<Object> changeDroneStatus(@PathVariable("drone_id") Long drone_id, @PathVariable("state") DroneState state) {
        ...
        if (fetchedDrone.getBatteryCapacity() < 25 && state == DroneState.LOADING) {
            return new ApiResponse<>("Drone battery is below 25%, Please charge before proceeding", HttpStatus.OK, fetchedDrone);
        }
